...
class SaleOrder(models.Model):
    ...
    @api.depends('partner_id')
    def _inv_count(self):
        self.count_invoice = self.env['account.invoice'].search_count([('partner_id', '=', self.partner_invoice_id.id)])
        # print "Function Call"
        return self.count_invoice

    @api.depends('partner_id')
    def _get_inv_date(self):

        dat = self.env['account.invoice'].search(
            [('partner_id', '=', self.partner_invoice_id.id), ('write_date', '<', self.date_order),
             ('state', '=', 'paid')])
        for i in dat:
            # print i
            self.date_data = i.write_date
            self.inv_date = i.date_invoice
    ...
    @api.depends('partner_id')
    def _get_inv_line(self):
        data = self.env['sale.order'].search(
            [('partner_id', '=', self.partner_invoice_id.id), ('write_date', '<', self.date_order),
             ('invoice_status', '=', 'invoiced'), ('state', '=', 'sale')])
        for j in data:
            self.record_id = j.ids
            self.inv_line = j
            break
        dat = self.env['sale.order.line'].search(
            [('order_partner_id', '=', self.partner_invoice_id.id), ('write_date', '<', self.date_order),
             ('invoice_status', '=', 'invoiced'), ('order_id', '=', self.inv_line)])
        line=[]
        for l in dat:
            val={
                'id_product':l.product_id.id,
                'uom_qty_product':l.qty_invoiced,
    ...
    care_setting = fields.Selection([('ipd', 'IPD'),
                                     ('opd', 'OPD')], string="Care Setting", default="opd")
    ...
    discount_type = fields.Selection([('none', 'PAID'),
                                      ('fixed', 'FREE')], string="Discount Type",
                                     default='none')
    ...
    count_invoice = fields.Integer(compute="_inv_count", string='INV', copy=False, store=True)
    inv_date = fields.Date(string='Invoice Date', compute='_get_inv_date')
    inv_line = fields.Integer(string='Record', compute="_get_inv_line")
    line_order = fields.One2many('pre.inv', 'id_order', string='Order Lines',
                        ...
                        for pack in picking.pack_operation_product_ids:
                            if pack.product_id.tracking != 'none':
                                ...
                                if lot_ids:
                                    ...
                                    operation_link_obj = self.env['stock.move.operation.link'].search(
                                        [('operation_id', '=', pack.id)], limit=1)
            ...
            for inv in obj.invoice_ids:
                ...
                if inv.state == 'paid':
                    ...
                    default_fields = account_payment_env.with_context(
                        {'default_invoice_ids': [(4, inv.id, None)]}).default_get(fields)
...
class PreInv(models.TransientModel):
    ...
    id_order = fields.Many2one('sale.order', string='Order Reference', index=True)

    id_product = fields.Many2one('product.product', string='Product', domain=[('sale_ok', '=', True)],
                                 change_default=True,store=False,readonly=True)
    uom_qty_product = fields.Float(string='Quantity',readonly=True)
    unit_price = fields.Float('Unit Price', default=0.0,readonly=True)

    price_subtotal = fields.Float(string='Subtotal', readonly=True, store=False)
