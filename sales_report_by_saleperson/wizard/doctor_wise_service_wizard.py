...
class ProductcategoryWizard(models.TransientModel):
    ...
    doctor=fields.Many2one('res.doctor',string="Doctor",required=False)
    @api.multi
    def sale_productcategory_report(self,data):

        return self.env['report'].get_action(self,'sales_report_by_saleperson.sale_productcate_doctor',data=data)